...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({ email, password });
    return {
      access_token: (res.body as TokenPairResponseBody).access_token,
      refresh_token: (res.body as TokenPairResponseBody).refresh_token,
    };
  }
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      expect((res.body as RegisterResponseBody).id).toBeDefined();
      expect((res.body as RegisterResponseBody).email).toBe('user@example.com');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect((res.body as RegisterResponseBody).email).toBe('me@example.com');
    });
